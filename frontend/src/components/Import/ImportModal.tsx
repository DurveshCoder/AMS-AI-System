                    ...
                    {step === 2 && preview && (
                        <>
                            <div className="mb-4">
                                ...
                                <h3 className="font-semibold text-slate-800 mb-2">Column Mapping</h3>
                                <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                                    {preview.headers.map((h: string) => (
                                        <div key={h} className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg text-sm">
                                            <span className="text-slate-500 truncate flex-1">{h}</span>
                                            <span className="text-xs px-2 py-0.5 bg-indigo-100 text-indigo-700 rounded-full font-medium">
                                                {preview.columnMap[h] || '(unmapped)'}
                                            </span>
                                        </div>
                                    ))}
                                </div>
                                    ...
                                    <thead>
                                        <tr className="bg-slate-50">
                                            {preview.headers.slice(0, 6).map((h: string) => (
                                                <th key={h} className="px-3 py-2 border text-left font-medium text-slate-600">{h}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {preview.previewRows.map((row: any, i: number) => (
                                            <tr key={i} className="hover:bg-slate-50">
                                                {preview.headers.slice(0, 6).map((h: string) => (
                                                    <td key={h} className="px-3 py-2 border text-slate-700 truncate max-w-[120px]">{row[h]}</td>
                                                ))}
                                            </tr>
                                        ))}
                                    </tbody>
                            ...
                            <div className="flex gap-3 mt-6">
                                <button onClick={() => setStep(1)} className="px-4 py-2 text-sm border rounded-lg hover:bg-slate-50">Back</button>
                                <button onClick={handleImport}
                                    className="flex-1 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg font-medium hover:opacity-90 text-sm">
                                    Proceed with Import
                                </button>
                            </div>
                        </>
